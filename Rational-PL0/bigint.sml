signature BIGINT =
  sig
    ...
    val sign : bigint -> int
    val len: bigint -> int 
    val concat: bigint * bigint -> bigint
end;
  ...
  struct
    ...
    fun listlt (a, b) = let
                        ...
                        in
                            ...
                            else if length(a) > length(b) then false
                            else helper (ra, rb)
                        end
    ...
    fun listadd (xs, ys, c) =
      ...
      in
        removeTrailingZeros(add(xs, ys, c))
      end 
    ...
    fun listsub (xs, ys, c) =
      ...
      in
        removeTrailingZeros(sub(xs, ys, c))
      end 
    ...
    fun mult(a,l) = let 
                        fun multhelper(a,[],c) = [c]
                        | multhelper(a,x::xs,c) = (((a*x)+c) mod 10)::multhelper(a,xs,((a*x)+c) div 10)  
                    in
                        multhelper(a,l,0)
                    end
                                             

    fun addmul(l1,l2) = let
                          fun addmulhelper([],l2,acc,ans) = ans
                            | addmulhelper(x::xs,l2,acc,ans) = addmulhelper(xs,l2,0::acc,listadd((acc@mult(x,l2)),ans,0))
                        in 
                          addmulhelper(l1,l2,[],[])
                        end

    fun mul (a,b) = case (a,b) of
        ((l1,0),(l2,0)) => (addmul(l1,l2),0)
      | ((l1,1),(l2,1)) => (addmul(l1,l2),0)       
      | ((l1,0),(l2,1)) => (addmul(l1,l2),1)
      | ((l1,1),(l2,0)) => (addmul(l1,l2),1)
      | (_,_)   => raise BadFormat 

    (*l1/l2*)
    fun divide(l1,l2) = if removeTrailingZeros(l2) = [0] then raise DivideByZeroException
                        else if listlt(l1,l2) then [0]
                        else listadd([1],divide(listsub(l1,l2,0),l2),0)
    ...
    fun gcd (a,b) = case (a,b) of
      ...
      | (([0],0),(l2,0)) =>  (l2,0)
      | ((l1,0),(l2,0))  => (
                              if listeq(l1,[1]) orelse listeq(l2,[1]) then ([1],0)
                              else if listeq(l1,l2) then (l1,0)
